...
pub fn draw_letter(
  font: &Vec<u8>,
  (letter_width, letter_height): (f32, f32),
  sender: &Recipient,
  recipient: &Recipient,
) -> (Vec<u8>, usize, usize) {
  let (x, y) = (
    (letter_width * 300.0).floor() as usize,
    (letter_height * 300.0).floor() as usize,
  );
  ...
  let sender_line_one = Text::new(
    (X_MARGIN, get_row_y(0, word_height) + Y_MARGIN),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    sender.get_letter_line_one(),
  );
  let sender_line_two = Text::new(
    (X_MARGIN, get_row_y(1, word_height) + Y_MARGIN),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    sender.get_letter_line_two(),
  );
  let sender_line_three = Text::new(
    (X_MARGIN, get_row_y(2, word_height) + Y_MARGIN),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    sender.get_letter_line_three(),
  );
  ...
  let mut recipient_line_one = Text::new(
    (0, 0),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    recipient.get_letter_line_one(),
  );
  recipient_line_one.pos = (
    get_centered_x(x, recipient_line_one.get_width()),
    get_row_y(START_ROW, word_height) + Y_MARGIN,
  );

  let mut recipient_line_two = Text::new(
    (0, 0),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    recipient.get_letter_line_two(),
  );
  recipient_line_two.pos = (
    get_centered_x(x, recipient_line_two.get_width()),
    get_row_y(START_ROW + 1, word_height) + Y_MARGIN,
  );

  let mut recipient_line_three = Text::new(
    (0, 0),
    [255, 0, 0, 0],
    font,
    word_height,
    1.0,
    recipient.get_letter_line_three(),
  );
  recipient_line_three.pos = (
    get_centered_x(x, recipient_line_three.get_width()),
    get_row_y(START_ROW + 2, word_height) + Y_MARGIN,
  );
